...
const SocialMarket: React.FC<SocialMarketProps> = ({ 
  ...
  const formatTimestamp = useCallback((timestamp: bigint | number): { formatted: string; raw: number } => {
  ...
  const parseContractData = useCallback((contractPools: Pool[]): Market[] => {
    return contractPools
      ...
      .map((pool, index) => {
        ...
        const { formatted: endTime, raw: endTimeTimestamp } = formatTimestamp(pool.end_time);
  ...
  const filterMarkets = useCallback((markets: Market[], filterType: FilterType): Market[] => {
    const now = Date.now();
  ...
  const marketCounts = useMemo(() => {
    ...
    return {
      ...
      active: rangeMarkets.filter(m => m.endTimeTimestamp && m.endTimeTimestamp > now).length,
      ended: rangeMarkets.filter(m => m.endTimeTimestamp && m.endTimeTimestamp <= now).length,
      endingSoon: rangeMarkets.filter(m => 
        m.endTimeTimestamp && 
  ...
  const fallbackMarkets = useMemo<Market[]>(() => [
    {
      ...
      endTime: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString("en-US", {
        year: "numeric",
        month: "short",
        day: "numeric",
      }),
    },
    {
      ...
      endTime: new Date(Date.now() + 2 * 24 * 60 * 60 * 1000).toLocaleDateString("en-US", {
        year: "numeric",
        month: "short",
        day: "numeric",
      }),
    },
    {
      ...
      endTime: new Date(Date.now() - 24 * 60 * 60 * 1000).toLocaleDateString("en-US", {
        year: "numeric",
        month: "short",
        day: "numeric",
      }),
  ...
  return (
    <section className="py-20 px-6 max-w-7xl mx-auto">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-xl text-gray-300 font-medium mb-6">Powered by Starknet</p>
        
        {/* Filter Tabs */}
        <div className="flex flex-wrap justify-center gap-2 mb-6">
          {[
            { key: 'all' as FilterType, label: 'All Markets', count: marketCounts.all },
            { key: 'active' as FilterType, label: 'Active', count: marketCounts.active },
            { key: 'ending-soon' as FilterType, label: 'Ending Soon', count: marketCounts.endingSoon },
            { key: 'ended' as FilterType, label: 'Ended', count: marketCounts.ended },
          ].map(({ key, label, count }) => (
            <button
              key={key}
        ...
        <div className="text-center py-16">
          <div className="max-w-md mx-auto">
            <div className="text-6xl mb-6">
              {filter === 'ended' ? '⏰' : filter === 'ending-soon' ? '⚡' : '🎯'}
            </div>
